...
size_label.pack(side = 'top', padx = 8, pady = 8, expand = True, fill = 'both')
...
def compress():
    ...
    for i in range(size):
        ...
        if value[i] ==[0]*size:
            continue
        while pos < size:
            while pos < size and value[i][pos] != 0:
                pos += 1
            if pos == size or (pos == size - 1 and value[i][pos] != 0):
                break
